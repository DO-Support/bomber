...
--              standard = SUM(rate * units) over 'Received')
--   Actual   = Reporting.vStockMovements (Despatch - Return, netted)
--   Compared in Rand, grouped by Job + cost sub type.
-- The date range selects JOBS by first production receipt; BOTH sides then use
-- the job's FULL totals across all dates (never windowed per row).
-- =============================================================================
...
WITH JobSet AS (
    ...
    HAVING MIN(p.MovementDate) >= @DateFrom AND MIN(p.MovementDate) < @DateTo
),
Header AS (
    SELECT p.fJobNumber AS JobNumber, MAX(p.JobDesc) AS JobDescription,
           MAX(p.Customer_Vendor) AS CustomerName,
           CAST(MIN(p.MovementDate) AS date) AS JobDate, SUM(p.fUnits) AS UnitsProduced
    FROM Reporting.vProductStockMovements AS p
    WHERE p.MovementType = 'Received' AND p.fJobNumber IN (SELECT fJobNumber FROM JobSet)
    GROUP BY p.fJobNumber
),
Standard AS (
    SELECT p.fJobNumber AS JobNumber, LTRIM(RTRIM(u.Material)) AS Material,
           SUM(u.Rate * p.fUnits) AS Standard_Cost
    FROM Reporting.vProductStockMovements AS p
    CROSS APPLY (VALUES
        ('Fabric', p.[Fabric]), ('Trims', p.[Trims]), ('Trims Zips', p.[Trims Zips]),
        ('Trims Tape', p.[Trims Tape]), ('Trims Labels', p.[Trims Labels]),
        ('Trims Cotton', p.[Trims Cotton]), ('Trims Packaging', p.[Trims Packaging]),
        ('Trims Elastic', p.[Trims Elastic]), ('Trims Buttons', p.[Trims Buttons]),
        ('Trims Press Studs', p.[Trims Press Studs]), ('Trims Drawcord', p.[Trims Drawcord]),
        ...
        -- ... extend with Fabric <vendor> sub types as needed
    ) AS u(Material, Rate)
    WHERE p.MovementType = 'Received' AND p.fJobNumber IN (SELECT fJobNumber FROM JobSet)
...
Actual AS (
    SELECT s.fJobNumber AS JobNumber, LTRIM(RTRIM(s.CostType)) AS Material,
    ...
    WHERE s.MovementType IN ('Despatch','Return') AND s.fJobNumber IN (SELECT fJobNumber FROM JobSet)
    GROUP BY s.fJobNumber, LTRIM(RTRIM(s.CostType))
),
StdUnits AS (
    SELECT r.JobNumber AS JobNumber, LTRIM(RTRIM(r.CostType)) AS Material,
...
SELECT
    ...
    h.JobDate, h.CustomerName, h.JobDescription,
...
FROM Standard AS st
FULL OUTER JOIN Actual AS ac ON ac.JobNumber = st.JobNumber AND ac.Material = st.Material
FULL OUTER JOIN StdUnits AS su ON su.JobNumber = COALESCE(st.JobNumber, ac.JobNumber)
   AND su.Material = COALESCE(st.Material, ac.Material)
LEFT JOIN Header AS h ON h.JobNumber = COALESCE(st.JobNumber, ac.JobNumber, su.JobNumber)
